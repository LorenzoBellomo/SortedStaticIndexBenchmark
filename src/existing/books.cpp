...
static std::ofstream write_byte_file("../../output/index_sizes/books.txt");
...
static void BM_PLEX_32(benchmark::State& state) {
    ...
    std::vector<uint32_t> lookups(data);
    random_unique(lookups.begin(), lookups.end(), M1);
    lookups.resize(M1);
    ...
    write_byte_file << "PLEX_32 - " << ts.GetSize() << std::endl;
...
static void BM_PLEX_128(benchmark::State& state) {
    ...
    for (const auto& key : data) tsb.AddKey(key);
    auto ts = tsb.Finalize();
    write_byte_file << "PLEX_128 - " << ts.GetSize() << std::endl;
...
static void BM_PLEX_8(benchmark::State& state) {
    ...
    for (const auto& key : data) tsb.AddKey(key);
    auto ts = tsb.Finalize();
    write_byte_file << "PLEX_8 - " << ts.GetSize() << std::endl;
...
static void BM_SDSL_EliasFano(benchmark::State& state) {
    ...
    write_byte_file << "SDSL_EliasFano - " << sdsl::size_in_bytes(ef)  << std::endl;
...
static void BM_RRR_vector_15(benchmark::State& state) {
    ...
    write_byte_file << "RRR_vector_15 - " << ((rrr.bits_per_element()*data.size()) / 8) << std::endl;
...
static void BM_RRR_vector_31(benchmark::State& state) {
    ...
    write_byte_file << "RRR_vector_31 - " << ((rrr.bits_per_element()*data.size()) / 8) << std::endl;
...
static void BM_RRR_vector_127(benchmark::State& state) {
    ...
    write_byte_file << "RRR_vector_127 - " << ((rrr.bits_per_element()*data.size()) / 8) << std::endl;
...
static void BM_LA_vector_6(benchmark::State& state) {
    ...
    write_byte_file << "LA_vector_6 - " << v1.size_in_bytes() << std::endl;
...
static void BM_LA_vector_8(benchmark::State& state) {
    ...
    write_byte_file << "LA_vector_8 - " << v1.size_in_bytes() << std::endl;
...
static void BM_LA_vector_10(benchmark::State& state) {
    ...
    write_byte_file << "LA_vector_10 - " << v1.size_in_bytes() << std::endl;
...
static void BM_LA_vector_12(benchmark::State& state) {
    ...
    write_byte_file << "LA_vector_12 - " << v1.size_in_bytes() << std::endl;
...
static void BM_LA_vector_opt(benchmark::State& state) {
    ...
    write_byte_file << "LA_vector_opt - " << v1.size_in_bytes() << std::endl;
...
static void BM_PGM_128(benchmark::State& state) {
    ...
    pgm::PGMIndex<uint32_t, epsilon> pgm_idx(data);
    write_byte_file << "PGM_128 - " << pgm_idx.size_in_bytes() << std::endl;
    ...
    for (auto _ : state) {
        for (auto q : lookups) {
            ...
            auto x = *std::lower_bound(lo, hi, q);
            benchmark::DoNotOptimize(x);
            assert(x == q);
        }
    }
...
static void BM_PGM_32(benchmark::State& state) {
    ...
    pgm::PGMIndex<uint32_t, epsilon> pgm_idx(data);
    write_byte_file << "PGM_32 - " << pgm_idx.size_in_bytes() << std::endl;
...
static void BM_PGM_8(benchmark::State& state) {
    ...
    pgm::PGMIndex<uint32_t, epsilon> pgm_idx(data);
    write_byte_file << "PGM_8 - " << pgm_idx.size_in_bytes() << std::endl;
...
static void BM_BranchlessPGM_128(benchmark::State& state) {
    ...
    write_byte_file << "BranchlessPGM_128 - " << pgm_idx.size_in_bytes() << std::endl;
...
static void BM_BranchlessPGM_32(benchmark::State& state) {
    ...
    const int epsilon = 32; // space-time trade-off parameter
    pgm::PGMIndex<uint32_t, epsilon, 4, true, 32, float> pgm_idx(data.begin(), data.end()-1);
    write_byte_file << "BranchlessPGM_32 - " << pgm_idx.size_in_bytes() << std::endl;
...
static void BM_BranchlessPGM_8(benchmark::State& state) {
    ...
    const int epsilon = 8; // space-time trade-off parameter
    pgm::PGMIndex<uint32_t, epsilon, 4, true, 32, float> pgm_idx(data.begin(), data.end()-1);
    write_byte_file << "BranchlessPGM_8 - " << pgm_idx.size_in_bytes() << std::endl;
...
static void BM_CSSTree(benchmark::State& state) {
    ...
    write_byte_file << "CSSTree - " << tree.size_in_bytes() << std::endl;
    for (auto _ : state) {
        for (auto q : lookups) {
            auto x = *tree.find(q);
            benchmark::DoNotOptimize(x);
...
static void BM_RMI_large(benchmark::State& state) {
    ...
    write_byte_file << "RMI_large - " << rmi_large_namespace::RMI_SIZE << std::endl;
    for (auto _ : state) {
        size_t err;
        for (auto q : lookups) {
...
static void BM_RMI_compact(benchmark::State& state) {
    ...
    write_byte_file << "RMI_compact - " << rmi_compact_namespace::RMI_SIZE << std::endl;
    for (auto _ : state) {
        size_t err;
        for (auto q : lookups) {
...
static void BM_std_vector(benchmark::State& state) {
    ...
    random_unique(lookups.begin(), lookups.end(), M1);
    lookups.resize(M1); 
    write_byte_file << "std_vector - " << (data.size()*sizeof(uint32_t)) << std::endl;
    for (auto _ : state) {
        for (auto q : lookups) {
...
static void BM_GammaCode_16(benchmark::State& state) {
    ...
    write_byte_file << "GammaCode_16 - " << sdsl::size_in_bytes(v_gamma) << std::endl;
...
static void BM_GammaCode_32(benchmark::State& state) {
    ...
    const auto dens = 32;
    wrapper_sdsl_enc_vector<sdsl::enc_vector_rank<sdsl::coder::elias_gamma_rank, dens>, std::vector<uint32_t>> v_gamma(data);
    write_byte_file << "GammaCode_32 - " << sdsl::size_in_bytes(v_gamma) << std::endl;
...
static void BM_DeltaCode_16(benchmark::State& state) {
    ...
    write_byte_file << "DeltaCode_16 - " << sdsl::size_in_bytes(v_delta) << std::endl;
...
static void BM_DeltaCode_32(benchmark::State& state) {
    ...
    const auto dens = 32;
    wrapper_sdsl_enc_vector<sdsl::enc_vector_rank<sdsl::coder::elias_delta_rank, dens>, std::vector<uint32_t>> v_delta(data);
    write_byte_file << "DeltaCode_32 - " << sdsl::size_in_bytes(v_delta) << std::endl;
...
static void BM_SIMDBtree(benchmark::State& state) {
    ...
    write_byte_file << "SIMDBtree - " << btree.size_in_bytes() << std::endl;
...
static void BM_SIMDBPlustree(benchmark::State& state) {
    ...
    write_byte_file << "SIMDBPlustree - " << btree.size_in_bytes() << std::endl;
...
static void BM_Roaring(benchmark::State& state) {
    ...
    write_byte_file << "Roaring - " << r1.getSizeInBytes() << std::endl;
...
static void BM_ALEX(benchmark::State& state) {
    ...
    write_byte_file << "ALEX - " << alex_.model_size() << std::endl;
